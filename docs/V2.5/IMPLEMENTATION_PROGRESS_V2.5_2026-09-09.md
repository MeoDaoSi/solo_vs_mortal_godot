...
### 2026-09-11 — Asset Integration Trial accessibility correction

- **Trạng thái:** `complete` (Presentation-only). Feature panel có vùng nội dung fixed-height, không cuộn nên route mở Trial trong panel không đảm bảo luôn thấy hoặc click được khi các controls runtime tràn vùng.
- **Sửa hẹp:** `Arena` nay tạo nút HUD **Mở Asset Integration Trial (F10)** ngay dưới Status panel, nằm ngoài Feature panel; phím `F10` gọi cùng viewer kể cả khi gameplay đang pause hoặc save writes bị block. Cả hai chỉ mở viewer đọc catalog, không gọi Simulation command và không chạm save/input gameplay.
- **Evidence:** `dotnet build solo_vs_mortal_godot.csproj --no-restore -v:minimal` pass 0 warning/0 error. Đây không phải gameplay/visual approval; không export lại hoặc chạy automated gameplay harness.

### 2026-09-11 — Asset Integration Trial catalog startup correction

- **Root cause:** first real Arena startup reached `CanonicalAssetCatalog.Load` and rejected 89 static Trial records because generator serialization emitted `frames` as an object, while the strict catalog contract requires an array. `_Ready()` stopped before any HUD control could exist; later null-reference messages were consequential only.
- **Sửa hẹp:** bọc nguyên frame record của đúng 89 static AssetId thành mảng một phần tử trong `data/v2.5/asset-catalog.v2.5.json` và Trial `provenance.json`. Không đổi pixel file, source/copied SHA, frame rect, 1000 ms duration, pivot, QA, authorization hoặc historical approval.
- **Evidence:** JSON structural check: catalog 149/149 and provenance 149/149 records have array `frames`; `dotnet build solo_vs_mortal_godot.csproj --no-restore -v:minimal` pass. Godot 4.7.2 headless startup of the actual Arena for five frames passed exit 0 with no catalog/script initialization exception. Chỉ là startup diagnostic kỹ thuật, không tự động gameplay test/acceptance. Legacy V6 save detection still rightly blocks gameplay mutations to preserve the original save, independently of the read-only Trial viewer.

### 2026-09-11 — Pixel Rendering Foundation

- **Trạng thái:** `implementation_complete — user visual review required`.
- **Đã thực hiện:** Chốt virtual viewport `640×360`, initial override `1280×720`, stretch `viewport`/`keep`/integer, nearest texture filtering và transform pixel snap; camera smoothing bị tắt. `BASE_TILE=32×32` được guard khi vẽ tile Trial, không chạm Simulation geometry/collision/navigation hay tự scale sprite art. HUD thường chỉ giữ region+HP/SP, currency, minimap, help/toast/map hint và nút **I · Chi tiết**. Inventory, progression, Hồn Phiên, danh sách Soul, action/integration/debug controls chuyển vào overlay có thể mở/đóng bằng `I`/`Esc`; World Map overlay cũng thu gọn vào virtual viewport.
- **Missing giữ hiển thị:** không substitute source art. Marker `MISSING:<AssetId>` còn rõ cho `world.ash_graves.npc`, `.arena`, `.secret` và 12 Skeleton Enemy clip không nằm trong Trial authorization (`idle.*`, `move.*`, `attack.w/e/n`, `hit.n`). `tiles.arena.ground` chưa có catalog mapping nên giữ fallback nền trung tính cũ, không giả là art hợp lệ.
- **Evidence:** `PIXEL_RENDERING_FOUNDATION_2026-09-11.md` ghi footprint Player/Enemy/Ally 64×64 logical (128×128 physical ở 2×), pickup 24×24 (48×48) và object renderer 84×84 nếu map được. Build và Godot startup chỉ là technical evidence; không chạy gameplay/parity/soak/acceptance harness, không thay renderer, gameplay, save hoặc `spec-lock`, không modify asset-production-system. Không có commit/push trong bước này.

### 2026-09-11 — Pixel Rendering Foundation: container HUD and NPC mapping audit

- **Trạng thái:** `implementation_complete — user visual review required`.
- **HUD:** `CanvasLayer/HudRoot` giờ là Control full-rect; top/bottom rows dùng anchor, `MarginContainer` 6 px, `HBoxContainer` và `PanelContainer`, thay vì panel/toạ độ author cho 1280. HUD thường chỉ giữ HP/SP đã round, resource compact, minimap 70×70, combat/Soul state, toast, map hint và nút chi tiết 72×22. Tutorial key list, inventory, detailed stats, Hồn Phiên, integration và controls development nằm trong overlay `I`/`Esc` có margin viewport 12/8 px. Font normal là 9–10 px; title 12–13 px.
- **Debug/missing:** NPC/object IDs, asset keys và text `MISSING:<AssetId>` bị tắt bởi `ShowPresentationDebug=false`; marker magenta không bị che để vẫn báo lỗi visual. Audit trace chốt `an/kha/linh` → `V25WorldLayout` → `world.ash_graves.npc` → catalog/provenance/copied PNG đều 0 entry → `RebuildMapTextures` skip → không có Godot texture → marker. Read-only asset workspace cũng không có record/output cùng AssetId; không substitute hoặc sửa art.
- **Render/evidence:** phát hiện `window/size/mode=3` fullscreen làm override bỏ qua (1920×1080), nên bỏ mode đó và giữ windowed override `1280×720` + aspect `keep`. Godot normal run exit `0` in `viewport=(640, 360); window=(1280, 720); baseTile=32`; build pass 0 warning/0 error. Đây chỉ là technical/UI proof, không gameplay acceptance. Không commit/push, không đổi Simulation, save, renderer, spec-lock hay asset-production-system.

### 2026-09-11 — Gameplay Readability + Movement Recovery

- **Trạng thái:** `implementation_complete — user manual movement/visual review required`.
- **Movement recovery:** InputMap WASD/arrow và `Arena → GameApplication → GameSession → PlayerSystem` đều đúng. Root cause là khi V2.5 store trống nhưng `user://solo_vs_mortal_save_v6.json` còn tồn tại, `TryLoad` thử restore JSON V6 không lossless vào canonical V2.5; catch đặt `_saveWritesBlocked`, khiến `_PhysicsProcess` gửi input zero và return mỗi tick. V6 file nay được giữ nguyên, fresh canonical V2.5 session chạy được và lưu riêng vào V2.5 store. V2.5 envelope lỗi vẫn giữ cơ chế block để không tự overwrite data hiện hành.
- **Readability/world scale:** Presentation lấy frame size+pivot canonical thay generic 84×84 center box. Hierarchy logical: debris 32 < rock 36 < chest 40 < pillar/grave 50 < Player/Enemy/Ally 64 < shrine 76 < landmark 80/tree-spire 82 < portal 92/arena target 96. Không scale lại sprite Player/monster/Soul, không đổi camera, collision hoặc navigation. Feet shadow và focal clearing ở Shrine/landmark/exit tăng contrast; bốn prop Trial exact AssetId làm camp grave dressing off-path, visual-only/pass-through để không tạo collision giả.
- **Remaining source gaps:** `world.ash_graves.npc`, `.arena`, `.secret` chưa có authorized catalog/provenance/PNG exact ID; 12 Skeleton Enemy clips vẫn ngoài Trial authorization. Những missing này tiếp tục marker magenta, không substitute. Đây là source asset pipeline/authorization gap, không phải Godot loader.
- **Evidence/safety:** `dotnet build solo_vs_mortal_godot.csproj --no-restore -v:minimal` pass 0 warning/0 error. Không run automated gameplay/parity/soak/acceptance, không đổi 640×360/renderer/HUD foundation/Simulation/save schema/spec-lock hoặc asset-production-system; không commit/push. Chi tiết tại `GAMEPLAY_READABILITY_MOVEMENT_RECOVERY_2026-09-11.md`.

### 2026-09-11 — Presentation motion and semantic-map corrective pass

- **Trạng thái:** `partial — prior visual acceptance remains failed; user review required`.
- **Animation audit:** cả bốn `player.base.move.s/w/e/n` đều resolve đúng six-frame SpriteFrames với 100 ms/frame và speed 1000, nhưng SHA-256 RGBA của mọi frame trong từng clip cho đúng 1 unique frame/6. Đây là classification C (source atlas frame visually identical), không phải thiếu AssetId. Không regenerate/đổi source art; DEBUG log `PRESENTATION_ANIMATION_PROBE` ghi asset/animation/frame count/duration/current frame/isPlaying khi user giữ phím để có runtime evidence thật.
- **Motion/cache:** terrain Ash Graves chuyển ra `AshGravesTerrainLayer` gồm chunk cache 16×16 tile (256 chunk); chỉ build khi region/layout thay, không bị `Arena.QueueRedraw()` từ snapshot/physics. Player/camera/enemy/Ally nay giữ target Simulation tách với rendered transform, nội suy ở `_Process` và snap pixel chỉ ở transform cuối. Không smoothing physics/collision hay đổi viewport/filter.
- **Scale/map:** thêm contract `presentation-visual-metrics.v2.5.json` (canvas, opaque bounds, ground pivot, intended footprint, optional collision field), validate catalog 23 entry; scale world lấy silhouette opaque thay generic canvas/type multiplier. Shrine dressing dùng exact Trial seal/banner/torch và grave cluster off-path; terrain dùng negative space, sparse ash patches, path corridor, focal clearing và broken Ruins foundations thay matrix lặp toàn màn hình. `bridge_segment` cataloged nhưng không dùng vì chưa có canonical water crossing; source có water masks generated ngoài locked `slice01` 161 scope nên không tự import.
- **Evidence/safety:** build pass 0 warning/0 error, static metrics audit pass. Computer-use connector không expose native Godot surface nên không fabricate screenshot hoặc claim visual acceptance. Không modify gameplay/save/schema/source art/asset-production-system/renderer/HUD/640×360; không commit/push. Chi tiết tại `PRESENTATION_MOTION_AND_SEMANTIC_MAP_AUDIT_2026-09-11.md`.
