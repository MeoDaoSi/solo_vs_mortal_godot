...
public partial class WorldMapUI : Control
{
    ...
    private void BuildLayout()
    {
        ...
        panel.OffsetLeft = 12; panel.OffsetTop = 12; panel.OffsetRight = -12; panel.OffsetBottom = -12;
        FancyUi.ApplyPanel(panel, main: true);
        AddChild(panel);
        var margin = new MarginContainer(); margin.AddThemeConstantOverride("margin_left", 10); margin.AddThemeConstantOverride("margin_top", 8); margin.AddThemeConstantOverride("margin_right", 10); margin.AddThemeConstantOverride("margin_bottom", 8); panel.AddChild(margin);
        var column = new VBoxContainer(); margin.AddChild(column);

        var header = new HBoxContainer(); column.AddChild(header);
        var title = new Label { Text = "BẢN ĐỒ THẾ GIỚI", ThemeTypeVariation = "HeaderMedium", SizeFlagsHorizontal = Control.SizeFlags.ExpandFill }; title.AddThemeColorOverride("font_color", FancyUi.TextBright); title.AddThemeFontSizeOverride("font_size", 13); header.AddChild(title);
        var close = new Button { Text = "Đóng", CustomMinimumSize = new Vector2(54, 20) }; close.Pressed += Close; FancyUi.ApplyButton(close); close.AddThemeFontSizeOverride("font_size", 9); header.AddChild(close);
        var hint = new Label { Text = "Chọn khu vực để xem điều kiện di chuyển.", Modulate = new Color(0.72f, 0.78f, 0.88f) }; hint.AddThemeFontSizeOverride("font_size", 9); column.AddChild(hint);

        var body = new HBoxContainer { SizeFlagsVertical = Control.SizeFlags.ExpandFill }; column.AddChild(body);
        _mapCanvas = new WorldMapCanvas { CustomMinimumSize = new Vector2(360, 244), SizeFlagsHorizontal = Control.SizeFlags.ExpandFill, SizeFlagsVertical = Control.SizeFlags.ExpandFill }; body.AddChild(_mapCanvas);
        var detailsPanel = new PanelContainer { CustomMinimumSize = new Vector2(210, 0) }; FancyUi.ApplyPanel(detailsPanel, main: false); body.AddChild(detailsPanel);
        var detailsMargin = new MarginContainer(); detailsMargin.AddThemeConstantOverride("margin_left", 8); detailsMargin.AddThemeConstantOverride("margin_top", 8); detailsMargin.AddThemeConstantOverride("margin_right", 8); detailsMargin.AddThemeConstantOverride("margin_bottom", 8); detailsPanel.AddChild(detailsMargin);
        var details = new VBoxContainer(); detailsMargin.AddChild(details);
        _selectionTitle = new Label { Text = "Chọn khu vực", ThemeTypeVariation = "HeaderMedium", AutowrapMode = TextServer.AutowrapMode.WordSmart }; _selectionTitle.AddThemeFontSizeOverride("font_size", 12); details.AddChild(_selectionTitle);
        _selectionDetails = new Label { AutowrapMode = TextServer.AutowrapMode.WordSmart, SizeFlagsVertical = Control.SizeFlags.ExpandFill }; _selectionDetails.AddThemeFontSizeOverride("font_size", 9); details.AddChild(_selectionDetails);
        _travelButton = new Button { Text = "Đi tới khu vực", CustomMinimumSize = new Vector2(0, 24) }; _travelButton.Pressed += TravelSelected; FancyUi.ApplyButton(_travelButton); _travelButton.AddThemeFontSizeOverride("font_size", 9); details.AddChild(_travelButton);
        var legend = new Label { Text = "Vàng: hiện tại\nĐỏ: chưa đến\nXanh: sẵn sàng", Modulate = new Color(0.72f, 0.78f, 0.88f) }; legend.AddThemeFontSizeOverride("font_size", 8); details.AddChild(legend);
    }
    ...
    private sealed partial class WorldMapCanvas : Control
    {
        ...
        public void Rebuild(IReadOnlyList<WorldMapRegionSnapshot> regions, string? selectedRegionId, Action<string> select)
        {
            ...
            _buttons.Clear();
            var center = Size / 2;
            var radius = Mathf.Min(Size.X, Size.Y) * 0.34f;
            foreach (var region in regions)
            {
                var button = new Button { Text = region.IsCurrent ? $"◆ {region.DisplayName}" : region.DisplayName, TooltipText = region.ShortDescription, CustomMinimumSize = new Vector2(78, 24), Size = new Vector2(78, 24) };
                button.AddThemeFontSizeOverride("font_size", 8);
                var position = center + new Vector2((float)region.WorldMapPosition.X * radius * 0.78f, (float)region.WorldMapPosition.Y * radius * 0.78f) - button.Size / 2;
        ...
        public override void _Draw()
        {
            ...
            DrawCircle(center, radius, new Color(0.035f, 0.08f, 0.13f, 0.96f));
            DrawArc(center, radius, 0, Mathf.Tau, 64, new Color(0.28f, 0.62f, 0.78f, 0.85f), 2);
            DrawArc(center, radius * 0.85f, 0, Mathf.Tau, 64, new Color(0.18f, 0.33f, 0.44f, 0.7f), 1);
            foreach (var region in _regions)
            {
                var position = center + new Vector2((float)region.WorldMapPosition.X * radius * 0.78f, (float)region.WorldMapPosition.Y * radius * 0.78f);
                DrawCircle(position, 5, region.IsCurrent ? new Color("#facc15") : region.IsAvailable ? new Color("#4ade80") : new Color("#f87171"));
            }
            DrawString(ThemeDB.FallbackFont, new Vector2(center.X - 38, 16), "VÒNG THẾ GIỚI", HorizontalAlignment.Left, -1, 10, new Color(0.72f, 0.85f, 0.92f));
        }
